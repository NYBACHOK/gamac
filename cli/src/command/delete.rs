...
use crate::operation::delete_script::delete_script;
use crate::operation::software_list::software_list;
use crate::{CliConfig, SystemId};
use std::process::Command;
use tokio::io::AsyncReadExt;

pub async fn delete_command(software_name: &str, config: &CliConfig) -> Result<String, Errors> {
    ...
    let script = delete_script(&SystemId(soft.uuid), config).await?;
